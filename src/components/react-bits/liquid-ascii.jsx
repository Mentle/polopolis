...
class Basin {
  ...
  separate(passes) {
    ...
    for (let i = 0; i < this.count; i++) {
      const ci = bound(Math.floor(this.pos[2 * i] * this.pInvH), 0, this.pNx - 1);
      const cj = bound(Math.floor(this.pos[2 * i + 1] * this.pInvH), 0, this.pNy - 1);
  ...
  solvePressure(iters, dt, relax, drift) {
    this.tension.fill(0);
    this.oldHx.set(this.hx);
    this.oldHy.set(this.hy);
...
const LiquidAscii = forwardRef(({
  ...
  useImperativeHandle(ref, () => ({
    splash() {
      ...
      // Big impulse from center
      const cx = simW * 0.5;
      const cy = simHt * 0.4;
      const cr = shortSide * 0.4;
      const force = cfgRef.current.cursorForce * 0.5;
      // Random direction each time
      const angle = Math.random() * Math.PI * 2;
      basin.impulse(cx, cy, Math.cos(angle) * force, Math.sin(angle) * force, cr);
    }
  }));
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (st.cursorX > -900) {
        ...
        const shortSide = Math.min(basin.gx * basin.step, basin.gy * basin.step);
      ...
      if (cfg.autoWave && idleMs > 2000) {
        ...
        const simHt = basin.gy * basin.step;
        const shortSide = Math.min(simW, simHt);
        const acr = cfg.cursorRadius * shortSide * 0.8;
        const ax = simW * (0.5 + 0.2 * Math.sin(t * 0.5));
        const ay = simHt * (0.3 + 0.1 * Math.sin(t * 0.7));
        const avx = Math.cos(t * 0.5) * 0.2 * simW * 0.5;
        const avy = Math.cos(t * 0.7) * 0.1 * simHt * 0.7;
        const strength =
          Math.min((idleMs - 2000) / 1000, 1) * cfg.cursorForce * 0.06;
